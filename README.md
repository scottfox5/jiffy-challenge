#Weekend Challenge 5

This is an application that uses the Giphy API. The user can search for a gif or click a button to return a random gif. The user can then comment on the gif and store it in their favorites. The user then can see the comments and urls of their favorite gifs.

##Author: Scott Fox

##Date: 5 Feb 2017

###Technology used:
HTML, CSS, Bootstrap, JavaScript, AngularJS, Node.js, Express, SQL, Git

###Setup:
The database used can be found in the file gifdatabase.sql.

npm init, npm install express --save, npm install body-parser --save, npm start, localhost:5555
